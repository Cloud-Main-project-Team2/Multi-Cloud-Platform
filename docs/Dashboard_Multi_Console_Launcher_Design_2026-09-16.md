...
### 3.1 왜 "한 페이지 안에서 터미널을 여는 것"은 불가능한가 (자세히)

실제 사용해 보면 열기 버튼을 누를 때 **각 CSP 셸이 브라우저의 별도 탭으로** 열린다(한 창의
여러 탭). "우리 페이지 한 화면 안에서" 3사 터미널을 보고 싶다는 요구가 자연스럽게 나오는데,
이는 **기술적으로 불가능**하다. "한 페이지 안"이 뜻할 수 있는 두 가지를 나눠 설명한다.

**(a) 공식 콘솔/CloudShell을 우리 페이지에 iframe으로 임베드 → 불가능**
- AWS/Azure/GCP 콘솔·CloudShell은 응답에 `X-Frame-Options: DENY` / CSP
  `frame-ancestors 'none'`을 실어 보낸다. 이걸 iframe 안에 넣으면 **브라우저가 렌더를 거부**한다
  (side-by-side든 탭이든, 프레임에 넣는 순간 동일하게 차단).
- 이는 우리 코드의 한계가 아니라 **각 CSP가 클릭재킹(clickjacking)을 막으려고 의도적으로 건
  보안 헤더**다. 우회 수단이 없고, 우회한다면 "편의보다 안정성" 원칙과 정면충돌한다.
- 따라서 **"임베드"가 아니라 "런처"** 로 설계했다: 우리는 *여는 조작*만 한 화면에 모으고,
  실제 터미널은 브라우저 탭에 맡긴다(§2·§3).

**(b) 공식 콘솔을 심는 대신, 우리가 직접 웹 터미널을 만든다 → 가능하지만 방향이 다른 대형 과제**
- 브라우저에 `xterm.js` 같은 터미널 UI를 띄우고 WebSocket으로 **우리 서버가 `aws`/`az`/`gcloud`
  CLI가 설치된 컨테이너에서 셸 세션을 돌려** stdio를 중계하면, 3사 터미널을 한 페이지에서
  탭/분할로 볼 수 있다.
- 그러나 이 방식은:
  - **서버가 각 사용자의 클라우드 자격증명을 쥐고, 인증된 CLI 세션을 대신 실행**해야 한다 →
    이 프로젝트가 지켜 온 **"로그인은 전적으로 사용자 몫 · 편의보다 안정성"** 원칙과 정면충돌.
  - 공식 CloudShell이 제공하는 **격리·감사·세션 수명관리를 우리가 새로 떠안는** 별도 시스템을
    운영·보안 책임까지 지고 구축하는 것이다(멀티클라우드 웹터미널 as-a-service).
  - AssumeRole 방향과 일부 맞닿지만 규모·리스크가 완전히 다른 과제라 **이번 범위에서 채택하지
    않는다.** 정말 필요하면 **보안 검토부터 시작하는 별도 과제**로 다룬다(§10).

**결론**: 공식 콘솔을 우리 페이지 안에 넣는 길은 없고, 브라우저 탭으로 여는 지금 방식이 원칙에
맞는 **안전한 표준 방법**이다. "각각의 탭으로 열린다"는 것은 버그가 아니라 이 제약의 정상적인
결과다.
