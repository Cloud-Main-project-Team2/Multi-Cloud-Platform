...
def test_create_job_without_runner_is_501(client, make_user, auth_header, db_session):
    ...
    # 12개 조합(ec2/rds/s3/cloudfront, vm/sql_database/storage_account/cdn, compute_engine/
    # cloud_sql/cloud_storage/cloud_cdn)이 2026-09-15 azure/cdn 러너 추가로 전부 러너를 갖게 돼,
    # 러너 없는 서비스를 검증하려면 존재하지 않는 조합을 새로 만들어야 한다(과거엔 azure/cdn을
    # 이 예시로 썼으나 2026-09-15 azure/cdn에 러너가 추가되면서 azure/waf로 교체함).
    _make_service(db_session, "azure", "waf", provisionable=True)
