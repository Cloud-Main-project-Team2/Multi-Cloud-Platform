# 작업 로그 — JWT 인증 방식을 적용하고 싶어. 어떻게 하면 되는지 제안해봐
...
> (작업명) **멀티 클라우드 연결 인증 전환 설계안**  
> _프롬프트 전문 생략 — 10070자_

### Claude

저장소 직접 확인을 마쳤습니다. 두 프롬프트를 하나의 검증으로 통합해, 프롬프트 2의 5단계 구조 + 최종 4버킷으로 답합니다. **모든 판정은 코드 근거를 답니다. 확인 못 한 건 확정하지 않습니다.**

---

## 0. 한 줄 결론 + 문서 전제 정정

설계안의 방향(장기 키 제거 → 위임 기반 임시 인증)은 이 코드베이스에 **개념적으로 부합하고 토큰 주입 계층 일부는 이미 준비돼 있으나**, 세 클라우드 모두 "그대로 바로 적용"은 불가능합니다. **가장 큰 미결정 변수 — 백엔드가 프로덕션에서 어디서 어떤 신원으로 도는가 — 가 저장소에 아예 정의돼 있지 않기** 때문입니다.

**설계안 §2의 스택 단서는 이 저장소와 틀립니다.** `package.json`·`wrangler`·`cloudflare`·React·Vinext는 **저장소에 존재하지 않습니다**(검색 0건). 실제는 **FastAPI(Docker 컨테이너)+정적 Vanilla JS(nginx)+Terraform CLI**입니다. 즉 설계안이 상상한 "Cloudflare/서버리스" 전제로 검증하면 안 됩니다.
...
- **프로덕션 호스팅/배포 신원 설정이 저장소에 전혀 없음**: wrangler·serverless·k8s·ECS/Lambda·CI-CD 배포 워크플로우 모두 0건. `docker-compose.yml`은 로컬 개발용. `backend/terraform/{aws,azure,gcp}`는 **고객 리소스 생성용 모듈**이지 서비스 호스팅 인프라가 아님. → **"백엔드가 어디서 도는가"는 코드상 미정.**
...
**문서 §2 대조**: 문서가 "미확정"으로 남긴 것들 중 — "Access Key 수집은 사용자 제공 정보"는 **코드로 사실 확인됨**(장기 비밀 저장 중). "백엔드 실행 위치 미확정"도 **사실**(배포 설정 없음). "package.json/Cloudflare 단서"는 **이 저장소엔 거짓**.

---

## 2. 클라우드별 적용 가능성 판정

세 클라우드 모두 공통 전제: "장기 비밀 완전 제거"는 백엔드가 **자기 자신의 신뢰된 클라우드 신원**(실행 Role/워크로드 ID)을 가질 때만 성립 — 이건 ①의 미정 사항에 종속됩니다.
...
→ **셋 중 어느 것도 "인프라 변경 없이 바로 적용"에 해당하지 않음.** 모두 (a) 임시 자격 획득 단계 신설 + (b) 백엔드의 신뢰된 first-party 클라우드 신원이 필요하고, (b)는 호스팅 미정이라 지금 확정 불가.
...
- **프로비저닝 러너 8개**의 `_credential_env`/`credentials_file`: `aws_provisioning.py`·`aws_rds_*`·`aws_s3_*`·`aws_cloudfront_*`·`azure_provisioning.py`·`azure_storage_*`·`azure_database_*`·`gcp_*` — 임시 자격/연동 토큰을 받도록.
...
3. **단일 프로세스 유지 vs 워커 분리**: 토큰 캐시·동시 갱신 설계가 갈림.
...
**D. 설계안 자체를 수정해야 함**
- **§2의 "package.json/React/Wrangler/Cloudflare" 단서 삭제** — 이 저장소 사실무근. "실제 스택 = FastAPI 컨테이너 + 정적 Vanilla JS + Terraform, 프로덕션 호스팅 미정"으로 교체.
  - 수정 문안 제안: *"실행 환경: 백엔드는 FastAPI 단일 프로세스(Docker), 클라우드 호출은 동일 프로세스의 BackgroundTasks + Terraform subprocess로 수행. 프로덕션 호스팅/실행 신원은 저장소에 미정의 — PoC 전 결정 필요. (이전 문서의 Cloudflare/Wrangler 언급은 본 저장소와 무관, 삭제.)"*
...
**PoC로 먼저 증명할 것**: (1) 정해진 호스팅에서 백엔드의 first-party 신원 확보, (2) AWS AssumeRole(External ID)로 임시키 → 기존 `_client`/`_credential_env`에 그대로 흘려 실제 읽기 1건, (3) Azure/GCP 각 1건 무(無)비밀 토큰 획득, (4) 토큰 만료 자동 갱신, (5) 타 조직 connection_id/External ID 불일치 거부.

---

원하시면 이 결론을 설계안 문서에 반영한 **수정 패치(§2·§7·§11 문안)** 를 만들거나, AWS 한 곳만 대상으로 한 **PoC 브랜치**(모델 필드 + AssumeRole 어댑터 + 기존 러너 연결)를 떠서 실제 적용 가능성을 코드로 증명해 드리겠습니다. 어느 쪽으로 갈까요?
