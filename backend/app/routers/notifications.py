"""알림 조회/읽음 처리 API — 사이드바 벨(🔔)이 조회한다.

알림 row 생성은 프로비저닝 라우터가 이미 담당한다(성공/실패 시 `_create_notification`).
이 라우터는 조회(GET)와 읽음 처리(read-all)만 제공한다. 소유권은 `notifications.user_id`로
확인한다(§15와 동일한 사용자 스코프).
"""
