...
> **🆘 그냥 처음으로 되돌리고 싶다면 (안전장치).**
> 충돌 해결이 꼬였거나 지금 뭘 하는지 모르겠으면, **작업을 취소하고 시작 전 상태로 되돌릴 수 있습니다.**
> 먼저 `git status`의 맨 윗줄을 보세요 — 지금 무슨 작업 중인지(`rebasing` / `merging`) 알려줍니다.
> ```bash
> git merge --abort     # merge(병합) 중 충돌이 났을 때 → 병합 취소
> git rebase --abort    # rebase(리베이스) 중 충돌이 났을 때 → 리베이스 취소
> ```
> 취소해도 이미 커밋해 둔 내용은 사라지지 않습니다. 되돌린 뒤 천천히 다시 시도하면 됩니다.
